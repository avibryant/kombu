...
function multiply(left: number, right: number): number {
  if (left == Infinity && right == 0.0) return left
  else if (left == 0.0 && right == Infinity) return right
  else if (left == -Infinity && right >= 0) return left
  else if (right == -Infinity && left >= 0) return right
...
function reciprocal(x: Bounds): Bounds {
  if (x.lower <= 0.0 && x.upper >= 0.0) return Unbounded
  else
    return {
      lower: 1.0 / x.upper,
      upper: 1.0 / x.lower,
...
function positivePow(x: Bounds, y: Bounds): Bounds {
  if (x.lower >= 0.0) return positivePositivePow(x, y)
  else if (x.upper <= 0.0) return negativePositivePow(x, y)
  else
    return or(
...
function negativePow(x: Bounds, y: Bounds): Bounds {
  return reciprocal(
    positivePow(x, { lower: y.lower * -1, upper: y.upper * -1 }),
  )
}
...
export function pow(x: Bounds, y: Bounds): Bounds {
  if (y.lower >= 0.0) return positivePow(x, y)
  else if (y.upper <= 0.0) return negativePow(x, y)
  else
    return or(
