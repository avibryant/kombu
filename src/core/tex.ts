...
function printNumBuf(n: t.Num, buf: String[], parens: boolean = false) {
  ...
  switch (n.type) {
    ...
    case t.NumType.Sum:
      ...
      if (n.k != 0) count += 1
      if (parens && count > 1) buf.push("(")

      const first = pos.shift()
      if (first) {
        if (first.a != 1) buf.push(first.a.toString())
    ...
    case t.NumType.Product:
      splitNodes(n.firstTerm, pos, neg)
      if (neg.length == 0) printProductBuf(pos, buf)
      else {
        buf.push("\\frac{")
        if (pos.length == 0) buf.push("1")
        else printProductBuf(pos, buf)
        buf.push("}{")
...
function splitNodes<T extends t.ProductTerm | t.SumTerm>(
  ...
  if (node.a > 0) pos.push({ x: node.x, a: node.a })
  else neg.push({ x: node.x, a: node.a * -1 })
  if (node.nextTerm) splitNodes(node.nextTerm, pos, neg)
}
...
function printBoundsBuf(b: Bounds, buf: String[]) {
  if (b.lower == -Infinity && b.upper == Infinity) return
