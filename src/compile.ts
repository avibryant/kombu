...
type Fn = (inp: Array<number>) => Array<number>

export function compile(inputs: Array<t.Param>, outputs: Array<t.Num>): Fn {
    function e(inp: Array<number>): Array<number> {
        const numCache = new Map<t.Num,number>()
        inp.forEach((v, i) => {
            numCache.set(inputs[i], v)
        })
